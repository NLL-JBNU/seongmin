...
class TranslationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #q_id = self.src_tokenizer.encode("<len>" + q_tokens) 원래 이리 되어 있는거 밑에로 바꿈
        q_id = self.src_tokenizer.encode(q_tokens)
        q_id.insert(0, self.src_tokenizer.index("<len>"))
    ...
    def load_data(self, file_path):
        srcs = []
        tgts = []
        f = open(file_path + "_src.txt", 'r', encoding="UTF-8")
        for line in tqdm(f):
            srcs.append(line.strip())
        f.close()

        f = open(file_path + "_tgt_morph.txt", 'r', encoding="UTF-8")
        for line in tqdm(f):
...
class TranslationModule(pl.LightningDataModule):
    def __init__(self, train_file, valid_file, src_tok, tgt_tok, max_len, batch_size=8, num_workers=5):
        super().__init__()
    ...
    def train_dataloader(self):
        ...
        # 여기는 좀 이해 안가요 ㅠㅠ 굳이 dataloader를 써야 하나요??
        # 일단 클린포머도 이렇게 구현하기는 했음
        return train
...
class Model(Base):
    def __init__(self, args, **kwargs):
        ...
        src_tok.read_vocab('/root/NAR_POS_TEST/data/src_vocab.txt')
        self.src_tokenizer = src_tok

        tgt_tok = MyTokenizer()
        tgt_tok.read_vocab('/root/NAR_POS_TEST/data/tgt_morph_vocab.txt')
        self.tgt_tokenizer = tgt_tok
...
if __name__ == '__main__':
    ...
    # 데이터 로더 돌림
    # 디바이스에 배치 넣어줌
        # = train_batch_start, training_setp, optimizer, backward(), step)_ 이런거 해준다는거...
